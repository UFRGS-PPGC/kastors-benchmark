...
#ifdef KLANG_VERSION
#pragma omp task depend(in:dA[0:A.mb*A.mb], dB[0:A.mb*A.mb], fake2[0:1]) depend(inout:dC[0:A.mb*A.mb]), depend(inout:fake1[0:A.mb*A.nb])
                        cblas_dgemm(CblasColMajor, (CBLAS_TRANSPOSE)PlasmaNoTrans, (CBLAS_TRANSPOSE)PlasmaNoTrans,
                                tempmm, tempnn, A.nb,
                                mzone, dA, ldam,
                                dB, ldak,
...
#pragma omp task depend(in:dA[0:A.mb*A.mb], dB[0:A.mb*A.mb], fake2[0:1]) depend(inout:dC[0:A.mb*A.mb], fake1[0:A.mb*A.nb])
    ...
    for (k = 0; k < min(A.mt, A.nt); k++)
    {
        ...
        for (n = 0; n < k; n++)
        {
            ...
            PLASMA_desc descA = plasma_desc_submatrix(A, tempk, n*A.nb, tempm, tempnn);
#ifdef KLANG_VERSION
#pragma omp task depend(inout:Aij[0:1]) depend(inout:fakedep) depend(in:dipiv[0:mintmp], prevSwap[0:A.lm*A.nb])

...
#pragma omp task depend(inout:Al[0:size])
        {
            ;
        }
/*
...
#pragma omp task depend(inout: Al[0:L*m*n]) shared(leaders)
            {
                double *W = (double *)alloca(L*sizeof(double));
                memcpy(W, &(Al[leaders[i]*L]), L*sizeof(double));
                CORE_dshiftw(leaders[i], 0, m, n, L, Al, W);
            }
            /*

    ...
    case PlasmaIPT_Panel:
        ...
        case PlasmaIPT_Panel:
            for (j=0; j<nt; j++) {
                ...
                for (i=0; i<mt; i++) {
                    Al = Ap + i*bsiz;
#pragma omp task depend(inout:Al[0:bsiz], Ap[0:psiz])
                    {
                        double *W = (double *)alloca(bsiz*sizeof(double));
                        CORE_dgetrip(mb, nb, Al, W);
                    }
                    /*
...
#pragma omp task depend(inout:Al[0:bsiz], A[0:size]) depend(in:Ap[0:size])
                    {
                        double *W = (double *)alloca(bsiz*sizeof(double));
                        CORE_dgetrip(mb, nb, Al, W);
                    }
        ...
        default:
            for (j=0; j<nt; j++) {
                ...
                for (i=0; i<mt; i++) {
                    Al = Ap + i*bsiz;
#pragma omp task depend(inout:Al[0:bsiz]) depend(in:Ap[0:psiz])
                    {
                        double *W = (double *)alloca(bsiz*sizeof(double));
                        CORE_dgetrip(mb, nb, Al, W);
                    }
    ...
    case PlasmaIPT_All:
        ...
        case PlasmaIPT_Panel:
            for (j=0; j<nt; j++) {
                ...
                for (i=0; i<mt; i++) {
                    Al = Ap + i*bsiz;
#pragma omp task depend(inout:Al[0:bsiz], Ap[0:psiz]) depend(in:A[0:size])
                    {
                        double *W = (double *)alloca(bsiz*sizeof(double));
                        CORE_dgetrip(mb, nb, Al, W);
                    }
        ...
        case PlasmaIPT_All:
            ...
            for (i=0; i<nprob; i++) {
                double *dA = &(A[i*bsiz]);
#pragma omp task depend(inout:dA[0:bsiz], A[0:size])
                    {
                        double *W = (double *)alloca(bsiz*sizeof(double));
                        CORE_dgetrip(mb, nb, dA, W);
                    }
                /*
...
#pragma omp task depend(inout:dA[0:bsiz], A[0:size])
        ...
        default:
            ...
            for (i=0; i<nprob; i++) {
                double *dA = &(A[i*bsiz]);
#pragma omp task depend(inout:dA[0:bsiz])
                    {
                        double *W = (double *)alloca(bsiz*sizeof(double));
                        CORE_dgetrip(mb, nb, dA, W);
                    }

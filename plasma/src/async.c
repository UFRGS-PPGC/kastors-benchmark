...
{
    if ((*sequence = (PLASMA_sequence *)malloc(sizeof(PLASMA_sequence))) == NULL) {
 ...
 **/
void plasma_sequence_flush(PLASMA_sequence *sequence, PLASMA_request *request, int status)
{
    sequence->request = request;
    sequence->status = status;
    request->status = status;
...
int PLASMA_Sequence_Flush(PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma_sequence_flush(sequence, request, PLASMA_ERR_SEQUENCE_FLUSHED);
    return PLASMA_SUCCESS;
}
